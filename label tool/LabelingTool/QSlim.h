...
extern  double triangleArea(const Vec3& v1, const Vec3& v2, const Vec3& v3);
#define ROOT3 1.732050807568877
#define FOUR_ROOT3 6.928203230275509
extern  double triangleCompactness(const Vec3& v1, const Vec3& v2, const Vec3& v3);
...
class ProxGrid
{
	...
	void maybe_collect_points(Vec3 *v, buffer<Vec3 *>& close,
		ProxGrid_Cell& cell);
public:
	ProxGrid(const Vec3& min, const Vec3& max, double dist);
	~ProxGrid() { cells.free(); }
	void addPoint(Vec3 *);
...
extern double boundary_constraint_weight;
...
extern double pair_selection_tolerance;
...
public:
	...
	Vertex   *newVertex(double x=0.0, double y=0.0, double z=0.0);
	Edge     *newEdge(Vertex *,Vertex *);
	Face *newFace(Vertex *, Vertex *, Vertex *);
	void killVertex(Vertex *);
	void killEdge(Edge *);
	void killFace(Face *);
	void reshapeVertex(Vertex *, double, double, double);
...
extern double quadrix_pair_target(const Mat4& Q,Vertex *v1,Vertex *v2,Vec3& candidate);

extern Vertex *decimate_last_v0;
extern Vertex *decimate_last_v1;
extern bool decimate_quadric(Vertex *v, Mat4& Q);
extern double decimate_min_error();
extern double decimate_max_error(Model& m);
extern double decimate_error(Vertex *);
extern void decimate_contract(Model& m);
extern void decimate_init(Model& m, double limit);
