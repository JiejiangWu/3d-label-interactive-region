...
ProxGrid::ProxGrid(const Vec3& lo, const Vec3& hi, double dist)
{
	cellsize = dist;
	cellsize2 = dist*dist;
	min = lo;
...
static void interp(double *proj, const double *p1, const double *p2,const double *p3, const double *bary)
{
...
static void ProjectPtri(const double *point,  const double *v1, const double *v2,  const double *v3, double *bary)
{
	int    i;
...
double __gfx_hoppe_dist(const Face3& f, const Vec3& v)
{
	Vec3 bary;
...
double boundary_constraint_weight = 1.0;
bool will_weight_by_area = false;
int placement_policy = PLACE_OPTIMAL;
double pair_selection_tolerance = 0.0;
Model M0;
...
Vertex *Model::newVertex(double x, double y, double z)
{
...
void Model::reshapeVertex(Vertex *v, double x, double y, double z)
{
	v->set(x, y, z);
}
...
double quadrix_pair_target(const Mat4& Q,Vertex *v1,Vertex *v2,Vec3& candidate)
{
...
static double pair_mesh_penalty(Model& M, Vertex *v1, Vertex *v2, Vec3& vnew)
{
...
double decimate_error(Vertex *v)
{
...
double decimate_max_error(Model& m)
{
	double max_err = 0;
...
void decimate_init(Model& m, double limit)
{
	int i,j;
